        ...
        if(it != mapVotedOutpoints.end()) {
            for (const auto& hash : it->second) {
                if(hash != txLockRequest.GetHash()) {
                    LogPrint(BCLog::IS, "CInstantSend::ProcessTxLockRequest -- Double spend attempt! %s\n", txin.prevout.ToString());
...
bool CInstantSend::CreateTxLockCandidate(const CTxLockRequest& txLockRequest)
{
    ...
    if(itLockCandidate == mapTxLockCandidates.end()) {
        ...
        LogPrint(BCLog::IS, "CInstantSend::CreateTxLockCandidate -- seen, txid=%s\n", txHash.ToString());
    }
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        if(nPrevoutHeight == -1) {
            LogPrint(BCLog::IS, "CInstantSend::Vote -- Failed to find UTXO %s\n", itOutpointLock->first.ToString());
            return;
        }

        int nLockInputHeight = nPrevoutHeight + 6/*Params().GetConsensus().nInstantSendConfirmationsRequired*/ - 2;

        int nRank;
        if(!mnodeman.GetMasternodeRank(activeMasternode.outpoint, nRank, nLockInputHeight, 0)) {
            LogPrint(BCLog::IS, "CInstantSend::Vote -- Can't calculate rank for masternode %s\n", activeMasternode.outpoint.ToString());
            ++itOutpointLock;
            continue;
        }
        ...
        if(nRank > nSignaturesTotal) {
            LogPrint(BCLog::IS, "CInstantSend::Vote -- Masternode not in the top %d (%d)\n", nSignaturesTotal, nRank);
            ++itOutpointLock;
            continue;
        }

        LogPrint(BCLog::IS, "CInstantSend::Vote -- In the top %d (%d)\n", nSignaturesTotal, nRank);
...
bool CInstantSend::ProcessNewTxLockVote(CNode* pfrom, const CTxLockVote& vote, CConnman& connman)
{
    ...
    if(!vote.IsValid(pfrom, connman)) {
        ...
        LogPrint(BCLog::IS, "CInstantSend::%s -- Vote is invalid, txid=%s\n", __func__, txHash.ToString());
        return false;
    }
    ...
    if(it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        ...
        LogPrint(BCLog::IS, "CInstantSend::%s -- Orphan vote: txid=%s  masternode=%s %s\n",
        ...
        if(itMnOV == mapMasternodeOrphanVotes.end()) {
            ...
            if(itMnOV->second > GetTime() && itMnOV->second > GetAverageMasternodeOrphanVoteTime()) {
                LogPrint(BCLog::IS, "CInstantSend::%s -- masternode is spamming orphan Transaction Lock Votes: txid=%s  masternode=%s\n",
    ...
    if (txLockCandidate.IsTimedOut()) {
        LogPrint(BCLog::IS, "CInstantSend::%s -- too late, Transaction Lock timed out, txid=%s\n", __func__, txHash.ToString());
        return false;
    }

    LogPrint(BCLog::IS, "CInstantSend::%s -- Transaction Lock Vote, txid=%s\n", __func__, txHash.ToString());
    ...
    LogPrint(BCLog::IS, "CInstantSend::%s -- Transaction Lock signatures count: %d/%d, vote hash=%s\n", __func__,
...
void CInstantSend::TryToFinalizeLockCandidate(const CTxLockCandidate& txLockCandidate)
{
    ...
    if(txLockCandidate.IsAllOutPointsReady() && !IsLockedInstantSendTransaction(txHash)) {
        ...
        LogPrint(BCLog::IS, "CInstantSend::TryToFinalizeLockCandidate -- Transaction Lock is ready to complete, txid=%s\n", txHash.ToString());
...
//    GetMainSignals().NotifyTransactionLock(*txLockCandidate.txLockRequest.tx);
    ...
    LogPrint(BCLog::IS, "CInstantSend::UpdateLockedTransaction -- done, txid=%s\n", txHash.ToString());
}
...
void CInstantSend::LockTransactionInputs(const CTxLockCandidate& txLockCandidate)
{
    ...
    LogPrint(BCLog::IS, "CInstantSend::LockTransactionInputs -- done, txid=%s\n", txHash.ToString());
}
...
bool CInstantSend::ResolveConflicts(const CTxLockCandidate& txLockCandidate)
{
    ...
    if(GetTransaction(txHash, txTmp, Params().GetConsensus(), hashBlock, true) && hashBlock != uint256()) {
        LogPrint(BCLog::IS, "CInstantSend::ResolveConflicts -- Done, %s is included in block %s\n", txHash.ToString(), hashBlock.ToString());
        return true;
    }
    ...
    LogPrint(BCLog::IS, "CInstantSend::ResolveConflicts -- Done, txid=%s\n", txHash.ToString());
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itLockCandidate != mapTxLockCandidates.end()) {
        ...
        if(txLockCandidate.IsExpired(nCachedBlockHeight)) {
            LogPrint(BCLog::IS, "CInstantSend::CheckAndRemove -- Removing expired Transaction Lock Candidate: txid=%s\n", txHash.ToString());
            std::map<COutPoint, COutPointLock>::iterator itOutpointLock = txLockCandidate.mapOutPointLocks.begin();
            while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(nCachedBlockHeight)) {
            LogPrint(BCLog::IS, "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  masternode=%s\n",
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(itOrphanVote->second.IsTimedOut()) {
            LogPrint(BCLog::IS, "CInstantSend::CheckAndRemove -- Removing timed out orphan vote: txid=%s  masternode=%s\n",
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsFailed()) {
            LogPrint(BCLog::IS, "CInstantSend::CheckAndRemove -- Removing vote for failed lock attempt: txid=%s  masternode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetMasternodeOutpoint().ToString());
            mapTxLockVotes.erase(itVote++);
        } else {
            ++itVote;
    ...
    while(itMasternodeOrphan != mapMasternodeOrphanVotes.end()) {
        if(itMasternodeOrphan->second < GetTime()) {
            LogPrint(BCLog::IS, "CInstantSend::CheckAndRemove -- Removing timed out orphan masternode vote: masternode=%s\n",
    ...
    LogPrint(BCLog::IS, "CInstantSend::CheckAndRemove -- %s\n", ToString());
}
...
{
    if(!fEnableInstantSend || GetfLargeWorkForkFound() || fLargeWorkInvalidChainFound) return false;
...
int CInstantSend::GetTransactionLockSignatures(const uint256& txHash)
{
    ...
    if(GetfLargeWorkForkFound() || fLargeWorkInvalidChainFound) return -2;
...
void CInstantSend::BlockConnected(const std::shared_ptr<const CBlock> &block, const CBlockIndex *pindex) {
    for (const auto& tx : block->vtx) {
        ...
        int nHeightNew = (!pindex ? -1 : pindex->nHeight);
        
        LogPrint(BCLog::IS, "CInstantSend::SyncTransaction -- txid=%s nHeightNew=%d\n", txHash.ToString(), nHeightNew);

        // Check lock candidates
        std::map<uint256, CTxLockCandidate>::iterator itLockCandidate = mapTxLockCandidates.find(txHash);
        if (itLockCandidate != mapTxLockCandidates.end()) {
            LogPrint(BCLog::IS, "CInstantSend::SyncTransaction -- txid=%s nHeightNew=%d lock candidate updated\n",
                    txHash.ToString(), nHeightNew);
            ...
            while(itOutpointLock != itLockCandidate->second.mapOutPointLocks.end()) {
                ...
                while(itVote != vVotes.end()) {
                    ...
                    LogPrint(BCLog::IS, "CInstantSend::SyncTransaction -- txid=%s nHeightNew=%d vote %s updated\n",
...
bool CTxLockRequest::IsValid() const
{
    ...
    if(tx->vin.size() > WARN_MANY_INPUTS) {
        LogPrint(BCLog::IS, "CTxLockRequest::IsValid -- WARNING: Too many inputs: tx=%s", ToString());
    }

    AssertLockHeld(cs_main);
    if(!CheckFinalTx(*tx)) {
        LogPrint(BCLog::IS, "CTxLockRequest::IsValid -- Transaction is not final: tx=%s", ToString());
        return false;
    }

    CAmount nValueIn = 0;

    int nInstantSendConfirmationsRequired = 6;//Params().GetConsensus().nInstantSendConfirmationsRequired;
    ...
    for (const auto& txin : tx->vin) {
        ...
        if(!GetUTXOCoin(txin.prevout, coin)) {
            LogPrint(BCLog::IS, "CTxLockRequest::IsValid -- Failed to find UTXO %s\n", txin.prevout.ToString());
            return false;
        }
        ...
        if(nTxAge < nConfirmationsRequired) {
            LogPrint(BCLog::IS, "CTxLockRequest::IsValid -- outpoint %s too new: nTxAge=%d, nConfirmationsRequired=%d, txid=%s\n",
    ...
    if(nValueIn - nValueOut < GetMinFee()) {
        LogPrint(BCLog::IS, "CTxLockRequest::IsValid -- did not include enough fees in transaction: fees=%d, tx=%s", nValueOut - nValueIn, ToString());
        return false;
    }
...
bool CTxLockVote::IsValid(CNode* pnode, CConnman& connman) const
{
    if(!mnodeman.Has(outpointMasternode)) {
        LogPrint(BCLog::IS, "CTxLockVote::IsValid -- Unknown masternode %s\n", outpointMasternode.ToString());
    ...
    if(!GetUTXOCoin(outpoint, coin)) {
        LogPrint(BCLog::IS, "CTxLockVote::IsValid -- Failed to find UTXO %s\n", outpoint.ToString());
        return false;
    }

    int nLockInputHeight = coin.nHeight + 6/*Params().GetConsensus().nInstantSendConfirmationsRequired*/ - 2;

    int nRank;
    if(!mnodeman.GetMasternodeRank(outpointMasternode, nRank, nLockInputHeight, 0)) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint(BCLog::IS, "CTxLockVote::IsValid -- Can't calculate rank for masternode %s\n", outpointMasternode.ToString());
        return false;
    }
    LogPrint(BCLog::IS, "CTxLockVote::IsValid -- Masternode %s, rank=%d\n", outpointMasternode.ToString(), nRank);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(nRank > nSignaturesTotal) {
        LogPrint(BCLog::IS, "CTxLockVote::IsValid -- Masternode %s is not in the top %d (%d), vote hash=%s\n",
...
bool CTxLockVote::IsExpired(int nHeight) const
{
    ...
    return (nConfirmedHeight != -1) && (nHeight - nConfirmedHeight > 24/*Params().GetConsensus().nInstantSendKeepLock*/);
}
...
{
    CInv inv(MSG_TX, txLockRequest.tx->GetHash());
    connman.ForEachNode([&inv](CNode* pnode) {
        pnode->PushInventory(inv);
    });

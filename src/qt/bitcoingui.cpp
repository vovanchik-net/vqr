...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Vqr address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and vqr: URIs)"));
  ...
  //  connect(coinsAction, SIGNAL(triggered()), this, SLOT(gotoCoinsPage()));
    ...
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Vqr addresses"));
    ...
    openAction->setStatusTip(tr("Open a vqr: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Vqr command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Vqr network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    ...
    if(secs < 90*60)
    {
        ...
        if (masternodeSync.IsSynced()) {
            ...
            while (!masternodeSync.IsBlockchainSynced()) masternodeSync.SwitchToNextAsset(*g_connman);
        }
    }
    else
    {
...
void BitcoinGUI::setAdditionalDataSyncProgress(int nSyncProgress)
{
    if(!clientModel)
        return;
...
//    updateProgressBarVisibility();
    ...
    if (masternodeSync.IsSynced()) {
        ...
        progressBar->setValue(nSyncProgress * 10000.0 + 0.5);
    }
...
{
    QString strTitle = tr("Vqr"); // default title

...
bool CCryptoKeyStore::IsLocked(bool fForPosOnly) const
{
    if (!IsCrypted()) {
        return false;
    }
    if(!fForPosOnly && fOnlyPosAllowed) return true;
    LOCK(cs_KeyStore);
    return vMasterKey.empty();
}

bool CCryptoKeyStore::Lock(bool fAllowPosOnly)
{
    if (!SetCrypted())
        return false;

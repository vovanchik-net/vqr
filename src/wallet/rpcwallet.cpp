...
double CalcMintingProbability(uint32_t nBits, int timeOffset, CAmount nValue, int64_t nTime) {
    int64_t nTimeWeight = std::min((GetAdjustedTime() - nTime) + timeOffset, Params().GetConsensus().nCoinAgeTick*255) - Params().GetConsensus().nCoinAgeTick*3;
...
UniValue listminting(const JSONRPCRequest& request) {
    ...
    int64_t minAge = Params().GetConsensus().nCoinAgeTick*3 / DAY;
    ...
    for (auto &out: vCoins) {
        ...
        int64_t nDayWeight = (std::min((GetAdjustedTime() - (uint32_t)nTime), Params().GetConsensus().nCoinAgeTick*255) - Params().GetConsensus().nCoinAgeTick*3) / DAY;
        ...
        if (((GetAdjustedTime() - nTime) / DAY) >= minAge) {
            status = "mature";
            attempts = GetAdjustedTime() - nTime - Params().GetConsensus().nCoinAgeTick*3;
        }
        ...
        obj.push_back(Pair("minting-probability-15min", CalculateMintingProbabilityWithinPeriod(nBits, 15, nValue, nTime)));
        obj.push_back(Pair("minting-probability-1h",    CalculateMintingProbabilityWithinPeriod(nBits, 60, nValue, nTime)));
        obj.push_back(Pair("minting-probability-6h",    CalculateMintingProbabilityWithinPeriod(nBits, 60*6, nValue, nTime)));
        obj.push_back(Pair("minting-probability-1d",    CalculateMintingProbabilityWithinPeriod(nBits, 60*24, nValue, nTime)));
        obj.push_back(Pair("minting-probability-7d",    CalculateMintingProbabilityWithinPeriod(nBits, 60*24*7, nValue, nTime)));
